...
// Delete a fragment's data/metadata from S3/dynamodb. Returns a Promise
async function deleteFragment(ownerId, id) {
  ...
  const { Bucket, Key: S3Key } = params;
  const { TableName, Key: DynamoDBKey } = paramsddb;
  try {
    // Delete data from S3
    await s3Client.send(command);
    logger.info({ Bucket, S3Key }, 'S3 object deleted successfully.');

    // Delete metadata from the dynamodb
    await ddbDocClient.send(commandddb);
    logger.info({ TableName, DynamoDBKey }, 'DynamoDB item deleted successfully.');
  } catch (err) {
    logger.error({ err }, 'Error deleting fragment data/metadata from S3/dynamodb');
